...
canvas.addEventListener('mousedown', (event) => {
    ...
    if (selectedTool === 'thin' || selectedTool === 'thick') {
        const newLine = new MarkerLine(event.offsetX, event.offsetY, lineWidth, selectedColor);
        lines.push(newLine);
    }
});
// draw
...
customStickerButton.addEventListener('click', () => {
    ...
    if (customEmoji) {
        availableStickers.push(customEmoji);
...
function setTool(tool: string) {
    ...
    toolPreview = { thickness: lineWidth, x: canvas.width / 2, y: canvas.height / 2 };
    updateToolSelection();
    canvas.dispatchEvent(new CustomEvent('drawing-changed'));
}
